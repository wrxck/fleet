---
title: Bot Commands
description: Complete reference for all fleet-bot commands
---
...
Inputs are validated: `/waf_whitelist_add` rejects malformed IPs/CIDRs and overly-wide ranges (e.g. `0.0.0.0/0`) that would effectively disable the WAF, and `/waf_rate` requires both the requests-per-second and burst values to be between **1 and 1,000,000** (a non-positive value that would silently switch rate limiting off is refused).
...
<Aside type="caution">
Commands marked **Destructive** require inline keyboard confirmation before executing. The bot sends a confirm/cancel button pair.
</Aside>
