...
| `allowedChatIds` | int64[] | Chat IDs the bot will listen to. Messages from other chats are silently dropped |
| `allowedSenderIds` | int64[] | **User** IDs authorised to issue commands. When set, only these users may run commands (even inside an allowed group). When empty, the bot **default-denies**: only the operator's private 1:1 chat is accepted, and it **refuses to start** if any `allowedChatIds` entry is a group/channel (negative ID). See [Bot Security](/bot/security/) |
...
| `webhookSecret` | string | Optional. Dedicated key for verifying inbound webhook HMAC signatures. When omitted it falls back to `password`; setting a distinct value means a leak of the API password (sent to the relay on every outbound call) cannot be used to forge inbound webhooks. Generate with `openssl rand -base64 32` |
