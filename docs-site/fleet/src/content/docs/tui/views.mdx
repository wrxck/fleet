...
The TUI has eight views — four top-level and four detail views.
...
A top-level view (Tab-cycled) that tails log output from all registered apps simultaneously. Each app's output is shown in a separate scrollable pane.

- Streams live output from `docker compose logs -f` for every app
- Use arrow keys to navigate between app panes
- Press **Esc** or **Tab** to leave this view
