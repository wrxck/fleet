...
The TUI has four top-level views you cycle through with **Tab**:
...
4. **Logs** — multi-app log tail (all registered apps simultaneously)

From any top-level view you can drill into detail views (app detail, single-app logs, secret editing) and return with **Esc**.
...
| Key | Action |
|-----|--------|
| `Tab` | Cycle between Dashboard, Health, Secrets, Logs |
