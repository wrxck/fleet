...
- `src/core/` — business logic (docker, systemd, nginx, secrets, health, deps, runners, backup, audit, notify)
- `src/registry/` — registry-driven command definitions, argument schema parsing (Zod), output rendering, and CLI context factory. New commands should use `defineCommand()` from this layer.
- `src/adapters/` — fleet adapter interfaces and runner adapters (remote SSH build hosts, scheduler, signals, notifier, detector)
